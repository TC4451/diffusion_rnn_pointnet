...
if __name__ == "__main__":
    ...
    logging.basicConfig(filename = f"Mar9_decoder_lr{params['lr_d']}_e{params['num_epochs_d']}_loss.log",
                        level = logging.DEBUG,
                        format = '%(asctime)s:%(levelname)s:%(name)s:%(message)s')
    # load the convolution part of pre-trained encoder
    path_point_net = "/mnt/VOL1/fangzhou/local/trainningcode/Zilin_PointNet-WPI3/Mar7_point_net_v2_Summation.pth"
    ...
    point_net.eval()
    PATH_f = f"/mnt/VOL1/fangzhou/local/trainningcode/Zilin_PointNet-WPI3/Mar8_f_rnn_v18_Summation_lr{params['lr_f']}_e{params['num_epochs_f']}.pth"
    PATH_d = f"Mar9_decoder_lr{params['lr_d']}_e{params['num_epochs_d']}.pth"
