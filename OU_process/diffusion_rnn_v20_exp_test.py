...
class DRNetTest(nn.Module):
    ...
    def forward(self, T, g_pixel_tensor):
        ...
        for t in timestep:
            theta = 0.005
            # set eq
            alpha_t = math.exp(-1*theta*(t+1))
            alpha_T = math.exp(-1*theta*(T+1))
            a_t = (1-alpha_t)/(1-alpha_T)
            t_prime = T-t 
            # print(f"a_t value: {a_t}")
...
def test_f(point_net, trained_f_net, trained_decoder, params):
    ...
    plt.grid(True)
    plt.savefig('Mar26_tsne_exp_y0.png')
    plt.close()
    ...
    # plt.ylabel('Component 2')
    plt.grid(True)
    plt.savefig('Mar26_tsne_exp_g_pixel.png')
...
if __name__ == "__main__":
    ...
    g_net.eval()
    PATH_f = f"Mar19_f_rnn_v20_exp_cor_theta0.005.pth"
    ...
    PATH_d = 'Mar25_decoder_exp_theta0.005.pth'
