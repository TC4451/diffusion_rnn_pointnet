...
class DRNet(nn.Module):
    ...
    def forward(self, T, g_cumsum, g_pixel_tensor, loss_fn, L):
        ...
        for t in timestep:
            ...
            a_T = (T+1)/(T+1)
            alpha_t = 1-a_t
            alpha_t_minus = 1-a_t_minus
            t_prime = T-t 
            # sigma_t = math.sqrt(a_t*(1-a_t))
            sigma_t = math.sqrt((1-alpha_t)*alpha_t)
            ...
            y_t = latent_y_0 - (1-alpha_t) * g_cumsum[:, :, t_prime-1] + sigma_t*epsilon
            # print(y_t)
            # calculate expected y_t
            expected_y_t_minus = latent_y_0 - (1-alpha_t_minus) * g_cumsum[:, :, t_prime]
